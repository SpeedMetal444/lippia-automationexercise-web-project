...
Feature: Products - Browse, Search, Categories, Brands and Reviews
  ...
  Scenario: User can view products by category
    Then The user verifies that categories are visible on left side bar
    When The user clicks on "Women" category
    And The user clicks on any category link under "Women" category
    Then The user verifies that category page is displayed and confirms text "WOMEN - TOPS PRODUCTS"
    When The user clicks on any sub-category link of "Men" category on left side bar
  ...
  Scenario: User can view products by brand
    When The user clicks on "Products" button
    Then The user verifies that Brands are visible on left side bar
    When The user clicks on any brand name
    Then The user verifies that user is navigated to brand page and brand products are displayed
    When The user clicks on any other brand link on left side bar
  ...
  Scenario: User can add review on product
    When The user clicks on "Products" button
    Then The user verifies that user is navigated to ALL PRODUCTS page successfully
    When The user clicks on "View Product" button
